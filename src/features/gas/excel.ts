...
const findRowByFirstColumn = (
	...
	for (const row of jsonData) {
		if (
			...
			row[columName].includes(rowName)
		) {
			return row;
		}
	}
...
export const getExcelData = async (file: File, values: Values) => {
	...
	const henryHubRow = findRowByFirstColumn(workbook, 0, henryHub);
	// const sheetRows = ["El Paso Permian", "Houston Ship Channel", "SoCal Border Avg.", "Waha"]
	const elPasoPermian = findRowByFirstColumn(workbook, 1, "El Paso Permian");
	const houstonShipChannel = findRowByFirstColumn(
		workbook,
		1,
		"Houston Ship Channel",
	);
	...
	dataBaseData.push(
		...createExcelData("HH", values, date, henryHubRow),
